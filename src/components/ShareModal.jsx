...
// ── Cloud save body ───────────────────────────────────────────────────────────
// Self-contained component. Only needs external data + outcome callbacks.

function CloudBody({ authToken, initialSlug, initialToken, content, title, onCloudSave, onAuthToken, showToast }) {
  ...
  const [savedEditToken, setSavedEditToken] = useState(initialToken || null);
  ...
  }, [content, title, onCloudSave]);

  const handleSaveClick = useCallback(() => {
    setError(null);
    if (authToken) doSave(authToken);
    else setStep('email');
  }, [authToken, doSave]);
...
function ShareModal({
  ...
  const { legacyUrl, legacyLen } = useMemo(() => {
    const result = generateShareUrl();
    return result ? { legacyUrl: result.url, legacyLen: result.length } : { legacyUrl: '', legacyLen: 0 };
  }, [generateShareUrl]);
  ...
  const cloudReadOnlyUrl = savedSlug
    ? `${window.location.origin}${window.location.pathname}?doc=${savedSlug}`
    : null;
  ...
  const handleNativeShare = useCallback(async () => {
    const url = cloudReadOnlyUrl || legacyUrl;
    try {
  ...
  }, [cloudReadOnlyUrl, legacyUrl, onClose, showToast]);
